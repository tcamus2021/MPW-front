...
const Travail = (): JSX.Element => {
	const [selectedTravail, setSelectedTravail] = useState<TravailType | null>(
		null,
	);
	...
	return (
		<div>
			...
			<div className="travail-page__container">
				<div className="travail-page__timeline-container">
					{historiqueTravail.map((travail) => (
						<TravailDetails
							key={travail.dateDebut}
							travail={travail}
							openDetails={() => setSelectedTravail(travail)}
						/>
					))}
				</div>
			</div>
			{!!selectedTravail && (
				<Modal
					isOpen={!!selectedTravail}
					triggerClose={() => setSelectedTravail(null)}
				>
