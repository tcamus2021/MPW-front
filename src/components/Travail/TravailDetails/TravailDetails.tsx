 ...
 * @return {JSX.Element}
 */
const TravailDetails = ({ travail, openDetails }: TravailDetailsProps): JSX.Element => {
    const intl = useIntl();
    return (
        <div className="travail-page__timeline-item">
            <div className="travail-page__timeline-item-entreprise">
                <h3 className="travail-page__timeline-item-title">{travail.entreprise}</h3>
                <img className="travail-page__timeline-item-image" src={travail.logoEntreprise} aria-hidden="true" />
            </div>
            <div className="travail-page__timeline-item-details">
                <h3 className="travail-page__timeline-item-title"><FormattedMessage id={travail.poste} /></h3>
                <p><FormattedMessage id={travail.contrat} /></p>
                <button
                    className="travail-page__timeline-item-details-button btn" 
                    onClick={openDetails}
                    title={intl.formatMessage({ id: 'parcours.professionnel.details.more.information' })}
                >
                    <img className="travail-page__timeline-item-details-button-image" src="information.svg" aria-hidden="true" />
            ...
            <div className="travail-page__timeline-item-time">
                <p><FormattedMessage id={travail.dateDebut} /></p>
                {travail.dateFin && (
