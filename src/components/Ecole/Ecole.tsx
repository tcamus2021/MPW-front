...
const Ecole = (): JSX.Element => {
	...
	const [openedIndex, setOpenedIndex] = useState<number | null>(
		indexToOpen ? Number.parseInt(indexToOpen) : null,
	);
	const handleOpenedIndexChange = (index: number | null): void =>
	...
	return (
		<div className="ecole-page">
			<BarreRetour />
			<h2>
				<FormattedMessage id="parcours.scolaire.title" />
			</h2>
			<div className="ecole-page__list">
				{ecoles.map((ecole, index) => (
					<EcoleDetails
						...
						openDetails={() => handleOpenedIndexChange(index)}
						ecole={ecole}
					/>
				))}
			</div>
